...
/*   Updated: 2024/01/31 00:37:19 by almichel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	{
		free(line);
		line = malloc((ft_strllen(buf, 0) + 1) * sizeof(char));
		if (!line)
			return (NULL);
...
char	*ft_strjoin_gnl(char *s1, char *s2)
{
	...
	if (s2 == 0 && s1)
		return (s1);
	tab = malloc(((ft_strllen(s1, 0) + ft_strllen(s2, 0)) + 1) * sizeof(char));
	if (!tab)
	{
		free(s1);
		free(s2);
