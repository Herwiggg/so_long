...
/*   Updated: 2024/01/31 00:37:35 by almichel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line.h"

int	ft_strllen(char *str, int nb)
{
	int	i;
...
char	*ft_check_n(char *str, int *error)
{
	...
	{
		dest = malloc((ft_strllen(str, 0) + 1) * sizeof(char));
		if (!dest)
		{
			*error = 1;
			return (NULL);
	...
	{
		dest = malloc((ft_strllen(str, 1) + 2) * sizeof(char));
		if (!dest)
		{
			*error = 1;
			return (NULL);
